 ...
 * @category    Plugin
 * @copyright   Copyright (c) 2000-2015, Nicolas Maillard ABSOLUTE Web
...
function init_gateway_monetico() {
	...
	class WC_Gateway_Monetico extends WC_Payment_Gateway {
		...
		public function generate_monetico_form( $order_id ) {
			...
			$montant = number_format(str_replace(",",".",$order->order_total),2,".","");
			// Ajout de filtres
			...
			$sReference = $order->id;
			$sMontant = $montant;
			$sDevise  = $monetico_settings['currency_code'];
			$sDate = date("d/m/Y:H:i:s");
			$sLangue = $monetico_settings['merchant_country'];
			$sEmail = $order->billing_email;
			$sTexteLibre = $order->billing_email;
			$sNbrEch = "";
			$sDateEcheance1 = "";
			$sMontantEcheance1 = "";
			$sDateEcheance2 = "";
			...
			/*
			* PATCH FOR GIT ISSUE 149
			* "when canceling the order, the credits are still added"
			*/
			$cancel_return = esc_url_raw( $order->get_cancel_order_url_raw() );
			$oEpt->sUrlKO = $cancel_return;
...
<?php		
			...
			if($monetico_settings['debug']=='yes'){
				...
				echo "<strong>url_retour :</strong> ".trailingslashit(get_bloginfo('wpurl'))."<br/>";
		...
		function check_monetico_response() {
			...
			if (isset($_GET['wc-api']) && $_GET['wc-api'] == 'WC_Gateway_Monetico'):
				...
				define ("MONETICOPAIEMENT_KEY", $monetico_settings['cle']);
				define ("MONETICOPAIEMENT_EPTNUMBER", $monetico_settings['tpe']);
				define ("MONETICOPAIEMENT_VERSION", $monetico_settings['version']);
				if($monetico_settings['monetico_mode']=="Test"):
					$url_serveur = "https://p.monetico-services.com/test/";
				else:
					$url_serveur = "https://p.monetico-services.com/";
		...
		function paiement_confirme($order, $sent_to_admin, $plain_text = false) {
			if ( ! $sent_to_admin && 'monetico' === $order->payment_method && $order->has_status( 'processing' ) ) {  /// $order->get_status()
		...
		function abw_txt_erreur_paiement($texte) {
			...
			$order = new WC_Order( $order_id );
			if ($order->payment_method == 'monetico' && ($order->status == 'pending'||$order->status == 'cancelled')) {
